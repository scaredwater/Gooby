...
then
	...
	do
		...
		then
			...
			rclone rc operations/movefile _async=true srcFs=Local: srcRemote="${UPLOADS}${FILE}" dstFs=${RCLONESERVICE}:${RCLONEFOLDER} dstRemote="${FILE}" --user gooby --pass Go0by > /dev/null
		else
