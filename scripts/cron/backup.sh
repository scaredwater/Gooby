...
/usr/bin/rclone rc operations/movefile _async=true srcFs=Local: srcRemote=/tmp/${BACKUP} dstFs=$RCLONESERVICE: dstRemote=/Backup/$(hostname)/${BACKUP} --user test --pass P@ssword > /dev/null
